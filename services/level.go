...
// GetWorldInfo returns this runtime's active world name and spawn
// coordinates. Spawn is nil when level.dat is missing or unreadable -- a
// server that has never generated its world yet is a normal state, not an
// error, so the endpoint still answers with the name alone.
//
// A method on ServerRuntime rather than a package function reading the fixed
// ServerDir: every other per-server read went this way in Phase 1, and it is
// what lets the same handler serve both /api/world and
// /api/servers/:sid/world without a second implementation.
func (rt *ServerRuntime) GetWorldInfo() types.WorldInfo {
	level := rt.levelName()
	info := types.WorldInfo{LevelName: level}
	if x, y, z, ok := readWorldSpawn(filepath.Join(rt.Dir, level)); ok {
		info.Spawn = &types.Coords{X: x, Y: y, Z: z}
	}
	return info
}

// levelName reads level-name from this runtime's server.properties, falling
// back to vanilla's "world" when the file is missing or doesn't set it -- the
// same default Minecraft itself uses, so the path we probe matches the one the
// JVM would have created.
func (rt *ServerRuntime) levelName() string {
	props, err := rt.GetServerProperties()
	if err != nil {
		return "world"
	}
	if name := strings.TrimSpace(props["level-name"]); name != "" {
		return name
	}
	return "world"
}
